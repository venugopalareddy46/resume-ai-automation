...
- The Kubernetes Secret manifest contains placeholders only. Create the real Secret with `kubectl create secret generic ...` as shown in the manifest comments.
- If any API key was previously committed or shared, rotate/revoke it immediately.
- The Kubernetes backend uses SQLite, so it is configured with **one replica**. For production horizontal scaling, migrate to PostgreSQL (or another server database) before increasing backend replicas.

## Local Docker Compose

The frontend build uses `http://localhost:8000` for the local Compose backend. Production CI/CD overrides `VITE_API_BASE_URL` with the HTTPS API domain.
